...
const PharmacyQuestionnaire = () => {
  ...
  const getStats = () => {
    ...
    // Count by Rx generation
    const rxCounts = {};
    allResponses.forEach(r => {
      const rx = r.rxGeneration || (r.status === 'current' ? 'นศ.ปัจจุบัน' : 'อื่นๆ');
            ...
            <div className="max-w-6xl mx-auto">
              ...
              {adminTab === 'dashboard' && (
                <div className="space-y-6">
                  ...
                  {/* Responses by Rx - Ranking เรียงจากมากไปน้อย */}
                  <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                    <h3 className="text-white font-bold mb-4">🏆 อันดับรุ่นที่ตอบมากที่สุด</h3>
                    <div className="space-y-2">
                      {(() => {
                        const rxEntries = Object.entries(getStats().rxCounts).sort((a, b) => b[1] - a[1]);
                        const maxRxCount = Math.max(...rxEntries.map(([, c]) => c));
                        return rxEntries.map(([rx, count], index) => {
                          ...
                          return (
                            <div key={rx} className="flex items-center gap-2">
                              ...
                              <div className="w-20 text-xs text-gray-300 font-medium text-right flex-shrink-0">
                                {rx}
                              </div>
                              <div className="flex-1 h-6 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.1)' }}>
                                <div className="h-full rounded-full flex items-center justify-end pr-2" style={{
                                  width: `${(count / maxRxCount) * 100}%`,
                          ...
                          {allResponses.map((r, i) => (
                            <tr key={r.id} className="border-t border-gray-700 hover:bg-white hover:bg-opacity-5">
                              ...
                              <td className="p-3 text-gray-300">
                                {r.preferredTime === 'morning' ? '🌅 เช้า' : r.preferredTime === 'afternoon' ? '🌤️ บ่าย' : '-'}
                              </td>
